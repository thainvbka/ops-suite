...
function NotificationChannels() {
  ...
  const testChannel = async (id) => {
    try {
      const token = localStorage.getItem("token");
      await axios.post(
        `${API_URL}/notification-channels/${id}/test`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        }
    ...
    <div className="alerts-page">
      <div className="page-header">
        ...
        <button className="btn-primary" onClick={() => setShowEditor(true)}>
          <Plus size={16} style={{ marginRight: "4px" }} /> Add Channel
        </button>
      </div>
            ...
            <div key={channel.id} className="channel-card">
              <div className="channel-header">
                <div
                  ...
                  style={{ background: getTypeColor(channel.type) }}
                >
...
function ChannelEditor({ channel, onClose, onSave }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ["slack", "webhook", "discord"].includes(formData.type) &&
      !formData.config.url
    ) {
    ...
    try {
      ...
      if (channel) {
        await axios.put(
          `${API_URL}/notification-channels/${channel.id}`,
          payload,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
      ...
      <div className="query-editor-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="modal-body">
          ...
          <div className="editor-section">
            ...
            {channel && (
              <p className="help-text">Type cannot be changed after creation</p>
            )}
          </div>

...
import { RefreshCw, Star, Droplet, Check } from "lucide-react";
...
export default function JuiceShopApp({ token }) {
  ...
  const getDifficultyLabel = (difficulty) => {
    const labels = {
      1: { text: "Easy", stars: 1 },
      2: { text: "Medium", stars: 2 },
      3: { text: "Hard", stars: 3 },
      4: { text: "Very Hard", stars: 4 },
      5: { text: "Extreme", stars: 5 },
      6: { text: "Insane", stars: 6 },
    };
    return labels[difficulty] || { text: "Unknown", stars: 0 };
  };
        ...
        <div className="juice-shop-title">
          ...
          <div>
            <h1 style={{ display: "flex", alignItems: "center", gap: "12px" }}>
              <Droplet size={32} /> OWASP Juice Shop
            </h1>
        ...
        {Object.entries(groupedByCategory).map(
          ([category, categoryChallenges]) => {
            ...
            const categoryProgress = Math.round(
              (categorySolved / categoryTotal) * 100
            );
                    ...
                    .map((challenge) => (
                      ...
                      >
                        ...
                        <div className="challenge-footer">
                          ...
                          >
                            {Array.from({
                              length: getDifficultyLabel(challenge.difficulty)
                                .stars,
                            }).map((_, i) => (
                              <Star key={i} size={12} fill="currentColor" />
                            ))}
                            <span>
                              {getDifficultyLabel(challenge.difficulty).text}
                            </span>
                          </div>
                        </div>
                      </div>
